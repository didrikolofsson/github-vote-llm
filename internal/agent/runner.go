...
func (r *Runner) implement(ctx context.Context, owner, repo string, issue *gogithub.Issue) (string, error) {
	...
	spin := spinner.New(fmt.Sprintf("Preparing workspace for issue #%d...", issue.GetNumber()))
	spin.Start()
	defer spin.Stop()
	...
	spin.UpdateMessage(fmt.Sprintf("Running Claude Code on issue #%d...", issue.GetNumber()))
	...
	// Stage, commit, push
	spin.UpdateMessage(fmt.Sprintf("Pushing changes for issue #%d...", issue.GetNumber()))
	...
	spin.UpdateMessage(fmt.Sprintf("Creating pull request for issue #%d...", issue.GetNumber()))
